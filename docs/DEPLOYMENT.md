...
**Adding or changing an env var after the first deploy? A new deployment is required — this is Vercel's own behavior, not a bug in this app.** Vercel snapshots whichever environment variables are configured *at the moment a deployment is built* and injects that snapshot into that deployment's Serverless Functions. Saving a new/edited value in the dashboard does **not** retroactively reach any deployment that already exists — only a deployment triggered *after* the save picks it up. This bit Issue #16: `SUPABASE_SERVICE_ROLE_KEY` was visibly configured for Production and Preview in the dashboard, yet one specific already-built deployment kept reporting it missing at runtime — consistent with that deployment predating the save. After saving any env var change, always trigger a fresh deployment (push a commit, or use Vercel's "Redeploy") before assuming the app picked it up. `GET /api/health` (below) reports whether the *currently running* deployment actually sees each required var — check it after every redeploy rather than assuming the dashboard state is what's live.
